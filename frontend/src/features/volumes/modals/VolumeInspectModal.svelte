<script lang="ts">
  ...
  async function loadInspect() {
    ...
    error = null;
    try {
      const data = await dockerApi.inspectVolume(volumeName);
      rawJson = data;
    } catch (err: any) {
    ...
    >
      ...
      <div class="px-5 py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between shrink-0 bg-slate-50/50 dark:bg-slate-800/30">
        <div class="flex items-center gap-2.5 min-w-0">
          <div class="p-2 rounded-xl bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 shrink-0">
          ...
          <div class="min-w-0">
            <div class="flex items-center gap-2">
              ...
              <span class="text-xs font-mono px-2 py-0.5 rounded bg-cyan-100 dark:bg-cyan-900/40 text-cyan-700 dark:text-cyan-300">
                JSON
              </span>
            </div>
      ...
      <div class="px-5 py-3 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-wrap items-center justify-between gap-3 shrink-0">
        ...
        <div class="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            class="w-full pl-9 pr-8 py-1.5 text-xs rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-cyan-500"
          />
          {#if searchQuery}
            <button
              onclick={() => (searchQuery = '')}
        ...
        <div class="flex items-center gap-2">
          ...
          >
            <RefreshCw class="w-3.5 h-3.5 {loading ? 'animate-spin text-cyan-500' : ''}" />
            <span>Recargar</span>
          </button>
      ...
      <div class="flex-1 overflow-auto bg-slate-950 text-slate-200 p-4 font-mono text-xs leading-relaxed select-text">
        {#if loading}
          <div class="h-64 flex flex-col items-center justify-center gap-3 text-slate-400">
            ...
            <span class="text-xs">Inspeccionando volumen en Docker daemon...</span>
          </div>
        {:else if error}
          <div class="h-64 flex flex-col items-center justify-center gap-2 text-rose-400 p-6 text-center">
            <span class="font-semibold text-sm">Error al inspeccionar volumen</span>
            <p class="text-xs max-w-md text-rose-300/80">{error}</p>
          </div>
        {:else if lines.length > 0}
          <pre class="whitespace-pre"><code>{#each lines as line, index}
<div class="table-row {searchResults.lineIndices.has(index) ? 'bg-cyan-950/50' : ''}"><span class="table-cell text-right pr-4 select-none text-slate-600 opacity-60 w-10">{index + 1}</span><span class="table-cell">{@html highlightLine(line, searchQuery)}</span></div>{/each}</code></pre>
        {:else}
