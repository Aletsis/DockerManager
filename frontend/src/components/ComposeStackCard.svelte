<script lang="ts">
  ...
  const isStackLoading = $derived(containersStore.actionLoading === `stack:${stack.name}`);
  ...
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pb-3">
    ...
    <div class="flex items-center gap-3 min-w-0">
      <button
        ...
        class="p-1 rounded-lg hover:bg-violet-100 dark:hover:bg-violet-900/40 text-slate-500 dark:text-slate-400 transition-colors cursor-pointer"
    ...
    <div class="flex items-center justify-between sm:justify-end gap-3 flex-shrink-0">
      ...
      <div class="flex items-center gap-1.5 text-xs">
        <span class="font-medium text-slate-600 dark:text-slate-300">
          <span class="font-bold text-emerald-600 dark:text-emerald-400">{stack.runningCount}</span>/{stack.totalCount} activos
        </span>
      </div>
      ...
      <div class="flex items-center gap-1.5">
        ...
        {:else}
          ...
          <!-- Restart Stack -->
          <button
            onclick={() => containersStore.handleRestartStack(stack.name)}
          ...
          <!-- Stop Stack -->
          <button
            onclick={() => containersStore.handleStopStack(stack.name)}
