...
func TestParseRepoTag(t *testing.T) {
	...
	for _, tt := range tests {
		repo, tag, dangling := docker.ParseRepoTag(tt.input)
...
func TestBuildContainerConfig(t *testing.T) {
	...
	_, _, err := docker.BuildContainerConfig(containerdomain.CreateSpec{Image: "   "})
	if err == nil {
		t.Errorf("Expected error for empty image, got nil")
	}

	// 2. Valid request with ports, volumes, and env
	req := containerdomain.CreateSpec{
		Image:         "nginx:alpine",
		Name:          "web_test",
	...
	cfg, hostCfg, err := docker.BuildContainerConfig(req)
	if err != nil {
	...
	invalidReq := containerdomain.CreateSpec{
		Image: "redis:alpine",
		Ports: []string{"invalid_port_spec"},
	}
	_, _, err = docker.BuildContainerConfig(invalidReq)
	if err == nil {
...
func TestCalculateContainerStats(t *testing.T) {
	...
	res := docker.CalculateContainerStats("c123", "my_container", stats)
